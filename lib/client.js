...
const debug = require('debug')('micro-whalla:client');
...
Client.prototype._send = function (serviceName, request, callback) {
  ...
  debug("send %s", request.toJSON(serviceName + self.id));
...
Client.prototype.broadcast = function (request, callback) {
  ...
  debug("broadcast %s : %o", serviceName, request);
